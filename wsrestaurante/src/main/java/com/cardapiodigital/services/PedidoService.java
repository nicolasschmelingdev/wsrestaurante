...
@Service
public class PedidoService extends PedidoRetorno {
	...
	promocaoService.setTxPromocoes(new ArrayList<>());
	...
	pedido.setTemPromocao(promocaoService.getTxPromocoes());

	Lanche lanche = lancheRepository.findById(request.getIdLanche())
		.orElse(null);
	...
	return (ingredientesAdicionais + lancheRepository.findById(request.getIdLanche())
		.get()
		.getPreco() - promocaoService.aplicarPromocoes(request)) * promocaoService.promoLight(request);
    }
}
